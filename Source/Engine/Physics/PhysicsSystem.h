...
#define VEC2_TO_B2VEC2(vec) (*(b2Vec2*)(&vec)) //converts our vec2 into box2d's vec2
#define B2VEC2_TO_VEC2(vec) (*(umbra::Vector2*)(&vec))
...
namespace umbra
{
	...
	{
	public:
		struct RigidBodyData //the way our physics body moves
		{
		...
		struct CollisionData //contains how heavy something is
		{
			vec2 size;
			float density = 1;
			float friction = 1;
