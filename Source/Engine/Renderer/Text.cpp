...
namespace umbra
{
	...
	{
		int width, height;
		SDL_QueryTexture(m_texture, nullptr, nullptr, &width, &height); //passing int pointers = &int
		...
		dest.x = (int)(position.x - (position.x / 2)); //should half x
		dest.y = (int)(position.y - (position.y / 2)); //should half y
		dest.w = (int)size.x;
		dest.h = (int)size.y;
